...
class _DateFilterScreenState extends State<DateFilterScreen>{
          ...
          child: Column(
            children: <Widget>[
              ...
              Container(
                  ...
                  child: _getCalendar()),
              Padding(padding: EdgeInsets.only(top: (_screenHeight * .1154)),),
              Center(
                child: Container(
  ...
  Widget _getCalendar(){
    ...
    var daysOfTheWeek = <String>[_stringResources.tSunday, _stringResources.tMonday, _stringResources.tTuesday, _stringResources.tWednesday, _stringResources.tThursday, _stringResources.tFriday, _stringResources.tSaturday];
    return CalendarCarousel(
      weekDays: daysOfTheWeek,
      onDayPressed: (DateTime date) {
        this.setState(() => _selectedDate = _addDate(_selectedDate, date));
      },
  ...
  List<DateTime> _addDate(List<DateTime> oldList, DateTime newDate) {
    if (oldList.length < 1) {
      ...
      if( oldList.length == 1){
        ...
        if(newDate.isBefore(firstSelectedDate))
          swap(oldList, 0, 1);
